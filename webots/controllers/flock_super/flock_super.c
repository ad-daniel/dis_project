...
/* Version:  	1.0                                                                           */
/* Date:     	15-Dec-18                                                                     */
/* Description: Simulation supervisor which is used to compute the performance of the robots and    */
/*  to explore different combination of parameters.                                                 */
/*                                                                       	                      */
...
#define TIME_STEP   64      // Length of time step [ms]
#define VMAX        0.1287  // Normalisation factor for one the metrics
...
#define VERBOSE_M   1
// Flags scenario
#define CROSSING    0  // Set 1 for the second scenario
#define OPTIMIZE    0  // Set 1 to start an optimization
...
WbDeviceTag receiver;                 // Receiver used to transmit the simulation weight to the robots
WbDeviceTag emitter;                  // Emitter used to transmit the simulation weight to the robots
...
// Good result used as default weights
double default_weight[5] = { 7, 7, 4, 0.01, 0.01 }; // Only the 3 firts weights are used
float default_RATIO = 1.5;

void send_default_params(){   // send default parameters to the robot when no optimization          
  char message[255]; 
...
void test_param(FILE *params, FILE *line_to_read){                              
  ...
  float RATIO = 1.5; 
  float weights[4] = {WEIGHT1, WEIGHT2, WEIGHT3, RATIO}; // Optimization SetUp of the Reynolds rules and the REYN_MIGR_RATIO
  int line = 1; 
