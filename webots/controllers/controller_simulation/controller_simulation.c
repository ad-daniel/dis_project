...
#define SWITCH_TO_AVOIDANCE_STATE  2900  //to change - took value from slides of DISAL on internet.
#define SWITCH_TO_REYNOLDS         2500  //not sure, juste choose to put an hysteresis compared to other state
...
void update_speed_pos(int *msl, int *msr){  
...
*/
void set_speed_with_state(int* bmsl, int* bmsr, int* max_sens, int* sum_sensors){    
  int maxDist = 0;
  sensor_to_keep();
  ...
  if(state == REYNOLDS_STATE){
    ...
    // Check state change, if the condition is met (e.g. close to an obstacle) change state
    if(maxDist > SWITCH_TO_AVOIDANCE_STATE){ 
      state = OBSTACLE_AVOIDANCE_STATE;
    }

  }

  else { // (state==OBSTACLE_AVOIDANCE_STATE)
    // Behavior
    ...
    if(maxDist < SWITCH_TO_REYNOLDS){ // hysteresis
...
int main(){ 
	...
	/*******************************************************************/
 	...
 	state = REYNOLDS_STATE; //must be in this state after alignment and before loop
	// Forever
	for(;;) {
		...
		//update speed and position of robot
		update_speed_pos(&msl, &msr);
    
    	//set speed depending on state: reynolds or obstacle avoidance
    	set_speed_with_state(&bmsl, &bmsr, &max_sens, &sum_sensors);
