...
void test_param(FILE *params, FILE *line_to_read){                              
  ...
  float init = 1; float resol = 1; float final = 10; int nb_param_tuned = 2; 
  ...
  asprintf (&file_name, "Test_parameters_2 WEIGHT Reynolds_[%.3f,%.3f,%.3f].csv", init, resol, final);
...
int main(int argc, char *args[]) {
	...
	for(;;) {
		...
		if (t % 1000 == 0) {
			...
			//Mathilde
			nb_repetition += 1; 
			fp = fopen("Reynolds_performance 4.csv" ,"a");
