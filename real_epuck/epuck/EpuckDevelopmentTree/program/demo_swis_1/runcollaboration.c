...
int sendLabPacket(int dataA, int dataB) {
	...
	if (e_radio_swis_send(RADIO_SWIS_DEFAULT_GROUP,0xff,packet,6)) {
		return 1;
	}
	return 0;
}
...
int LabPacketReady(int* dataA, int* dataB) {
	unsigned int size;
	unsigned char packet[RADIO_SWIS_MAXSIZE];
...
int run_collaboration(void) {
	...
	e_init_motors();
	e_init_prox();
	e_radio_swis_init(RADIO_SWIS_DEFAULT_GROUP,10,RADIO_SWIS_HW_ATTENUATOR_25DB, COM_RANGE );
